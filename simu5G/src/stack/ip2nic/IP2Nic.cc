...
void IP2Nic::initialize(int stage)
{
   ...
   /*     else if (nodeType_ == drone){

            cModule *drone = getParentModule()->getParentModule();

            masterId_ = drone->par("masterId");
            nodeId_ = binder_->registerNode(drone, nodeType_, masterId_);

            if (drone->hasPar("nrMasterId") && (int)drone->par("nrMasterId") != 0)   // register also the NR MacNodeId
            {
                nrMasterId_ = drone->par("nrMasterId");
                nrNodeId_ = binder_->registerNode(drone, nodeType_,nrMasterId_, true);
            }
            else
                nrNodeId_ = 0;
