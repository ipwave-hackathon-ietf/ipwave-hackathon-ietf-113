...
namespace inet {
//std::map<std::string, Coord> NodesPos;
...
void StationaryMobility::initialize(int stage)
{
 ...
 /*   if (strcmp(getParentModule()->getName(), "dst") == 0){
        std::map<std::string, Coord>::iterator it = NodesPos.find(getParentModule()->getFullName());
        if (it == NodesPos.end()){
            Coord InPos;
...
//            NodesPos.push_back(InPos);
            NodesPos.insert(std::make_pair(getParentModule()->getFullName(), InPos));
...
/*
std::map<std::string, Coord> StationaryMobility::returnDestinations(){
    return NodesPos;
}
*/
